...
- **AWS CDK v2** (`aws-cdk-lib ^2.254.0`) with TypeScript
- **`@orcabus/platform-cdk-constructs`** — internal shared constructs library (pinned to `1.5.4`)
- **`@aws-cdk/aws-lambda-python-alpha`** — Python Lambda bundling via `PythonUvFunction` (uses `uv`)
- **cdk-nag** — CDK security/compliance rule checks in tests
...
- **Amazon EventBridge** — event bus `OrcaBusMain`, source `orcabus.dragenwgtsrna`
- **AWS SSM Parameter Store** — configuration under `/orcabus/workflows/dragen-wgts-rna/`
- **AWS Schemas Registry** — JSON schema validation for draft events

## Package Manager

**pnpm** (v11.1.2). Always use `pnpm`, never `npm` or `yarn`.
...
- **Jest** with `ts-jest` for CDK infrastructure tests
