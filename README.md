...
WORKFLOW_VERSION="4.4.4"  # Name of the version were currently using
...
get_pipeline_id_from_workflow_version(){
  ...
  aws ssm get-parameter \
    --name "/orcabus/workflows/dragen-wgts-rna/pipeline-ids-by-workflow-version/${workflow_version}" \
    --output json | \
  jq --raw-output \
    '.Parameter.Value'
}
